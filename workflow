{
  "name": "🏛️ Generate museum social media copy from Google Sheets with Claude AI - Instagram & Facebook (Italian)",
  "nodes": [
    {
      "parameters": {
        "rule": {
    ...
    {
      "parameters": {
        "content": "## Genera contenuti social per il tuo con museo con AI\n\n### Come funziona\n1. Il trigger programmato si attiva ogni giorno alle 8:00\n2. Legge e filtra dal calendario editoriale in Google Sheets le righe con Stato = \"da processare\"\n3. L'AI genera copy personalizzati per Instagram e Facebook\n4. Scrive i post generati nel foglio e aggiorna Stato = \"processato\"\n\n### Setup\n1. Scarica e copia sul tuo drive il **template Google Sheets** (link nella descrizione)\n2. Configura **credenziali Google e API Anthropic**\n3. **Nodo Leggi**: inserisci URL del foglio → seleziona Sheet → Filters: Column = \"Stato\", Value = \"da processare\" → Options → \"When Filter Has Multiple Matches\" → **Return All Matches**\n4. **Nodo Aggiorna**: usa stesse credenziali e URL → Column to Match On = \"row_number\" → **CRITICO** mapping colonne (icona fx per expressions):\n   - row_number: `{{ $json.row_number }}`\n   - Instagram: `{{ $json.output.instagram }}`\n   - Facebook: `{{ $json.output.facebook }}`\n   - Stato: `processato` (fixed)\n6. **Testa**: Execute Workflow → verifica Instagram/Facebook popolati\n7. **Attiva**: toggle Active\n\n### Personalizzazioni\n**Cambiare modello AI:**\nDefault: Claude 3 Haiku (~$0.0007/post). Per qualità superiore: \"Anthropic Chat Model\" → Model → \"Claude 3.5 Sonnet\" (~$0.003/post).\n\n**Aggiungere piattaforme (LinkedIn, X...):**\nSe non hai esperienza di prompt engineering, chiedi a un AI di modificare il prompt → aggiungi chiave in \"Structured Output Parser\" → aggiungi mapping in \"Aggiorna\" → aggiungi colonna nel Sheets.\n",
        "height": 800,
        "width": 624
      },
